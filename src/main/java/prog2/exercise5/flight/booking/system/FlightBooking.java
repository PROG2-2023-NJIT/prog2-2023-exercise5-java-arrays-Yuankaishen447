...
public TripDestination getDestinationAirport(){
    ...
    public void setTicketNumber(int j) {
        ...
        switch (bookingClass) {
            case FIRST:
                ticketNumber[j] = ticketNumber[j] + "F";
                break;
            case BUSINESS:
                ticketNumber[j] = ticketNumber[j] + "B";
                break;
            case ECONOMY:
                ticketNumber[j] = ticketNumber[j] + "E";
                break;
        }
        for (int i = 0; i < 4; i++) {
        ...
        this.ticketNumber[j] = ticketNumber[j] + "DOM";
        ...
        if (tripSource == TripSource.NANJING && tripDestination == TripDestination.OULU) {
            this.ticketNumber[j] = ticketNumber[j] + "INT";
        }
